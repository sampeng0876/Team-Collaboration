...
def login():
    ...
    check_container = check_container.get()
    get_appt = get_appt.get()
    
    print(check_container,get_appt)
    # root.destroy()

frame = customtkinter.CTkFrame(master=root)
frame.pack(pady=20, padx=60, fill="both", expand=True)

label = customtkinter.CTkLabel(master=frame, text="One Click Solution",font=("Roboto", 24,'bold'))
label.pack(pady=12, padx=10)

check_container = customtkinter.CTkButton(master=frame, text="Check Container")
check_container = check_cont
check_container.pack(pady=12, padx=10)

get_appt = customtkinter.CTkButton(master=frame, text="Get Appt")
get_appt.pack(pady=12, padx=10)
